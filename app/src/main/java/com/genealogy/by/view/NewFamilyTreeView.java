...
import com.genealogy.by.db.FamilyDBHelper;
import com.genealogy.by.interfaces.OnFamilyClickListener;
...
public class NewFamilyTreeView extends ViewGroup {
    ...
    private static final int AVATAR_MALE = R.mipmap.ic_avatar_male;//男性默认头像
    private static final int AVATAR_FEMALE = R.mipmap.ic_avatar_female;//女性默认头像
    ...
    private View createFamilyView(FamilyBean family, int left, int top) {
        ...
        Glide.with(getContext())
                .load(url)
                .apply(new RequestOptions().error(SEX_FEMALE.equals(sex) ? AVATAR_FEMALE : AVATAR_MALE))
                .into(ivAvatar);
        if (family.isSelect()) {
